...
public class Pro158 {
     ...
     * @return    The number of actual characters read 实际读取的字符数
     */
    int size = 0 , i = 0; //i记录本次调用结束后 指针停留在 buf4 的位置
    char[] buf4 = new char[4];
    public int read(char[] buf, int n) {
        int idx = 0;
        while (idx < n){
            if (i == size){
                size = read4(buf4); //read4每次读4个，不够4个按实际算
                i = 0;
                if (size == 0) break;
            }
            for (;i < size && idx < n;) buf[idx++] = buf4[i++]; //若i < size时就停止，记录此时i的位置，用于下一轮遍历
        }
        return idx;
    }
