...
public class Text {
     ...
     * 一、通用情况 这个想法基于如下问题：给定一个表示每天股票价格的数组，什么因素决定了可以获得的最大收益？
     *   相信大多数人可以很快给出答案，例如「在哪些天进行交易以及允许多少次交易」。这些因素当然重要，在问题描述中也有这些因素。
     *   然而还有一个隐藏但是关键的因素决定了最大收益，下文将阐述这一点。
     *   首先介绍一些符号：
     *           用 n 表示股票价格数组的长度
     *           用 i 表示第 i 天（i的取值范围是 0 到 n - 1）
     *           用 k 表示允许的最大交易次数；
     *           用 T[i][k]表示在第 i 天结束时，最多进行 k 次交易的情况下可以获得的最大收益。
     * 基准情况：T[-1][k] = T[i][0] = 0, 表示没有进行股票交易时没有收益（注意第一天对应 i = 0，因此 i = -1 表示没有股票交易）。
     * 现在如果可以将 T[i][k] 关联到子问题，例如 T[i - 1][k]、T[i][k - 1]、T[i - 1][k - 1]等子问题，就能得到状态转移方程，并对这个问题求解。如何得到状态转移方程呢？
     *
     * 最直接的办法是第 i 天可能的操作。有多少个选项？答案是三个：买入、卖出、休息。
     * 应该选择哪个操作？ 答案是：并不知道哪个操作是最好的，但是可以通过计算得到选择每个操作可以得到的最大收益。
     * 假设没有别的限制条件，规定不能同时进行多次交易，因此如果决定
     *     第 i 天【买入】，在【买入】之前必须有 0 份股票；
     *     第 i 天【卖出】，在【卖出】之前必须恰好持有 1 份股票。
     *     持有股票的数量是上下提及的隐藏因素，该因素影响第 i 天可以进行的操作，进而影响最大收益。
     *  因此对 T[i][k] 的定义需要分成两项：
     *     （1）T[i][k][0] 表示在第 i 天结束时，【最多】进行 k 次交易且在进行操作后持有 0 份股票的情况下可以获得的最大收益；
     *     （2）T[i][k][1] 表示在第 i 天结束时，【最多】进行 k 次交易且在进行操作后持有 1 份股票的情况下可以获得的最大收益。
     *  使用新的状态表示之后，可以得到可以基准情况和状态转移方程。
     *  基准情况：
     ...
     *      T[i - 1][k][0] 是休息操作可以得到的最大收益，T[i - 1][k][1] + prices[i] 是卖出操作可以得到的最大收益。
     *      注意到允许的最大交易次数是不变的，因为每次交易包含两次成对的操作， 买入和卖出。只有买入操作会改变允许的最大交易次数。
     *  对于状态转移方程中的，
     *      T[i][k][1],第 i 天进行的操作只能是 休息或买入，因为在第 i 天结束时持有的股票数量是 1。
     *      T[i - 1][k][1] 是休息操作可以得到的最大收益，T[i - 1][k - 1][0] - prices[i] 是买入操作可以得到的最大收益。
     *      注意到允许的最大交易次数减少了一次，因为每次买入操作会使用一次交易。
     *  （有买入一定会有卖出，交易次数加一，最大交易次数减一）
     *  为了得到最后一天结束时的最大收益，可以遍历股票价格数组，根据状态转移方程计算 T[i][k][0] 和 T[i][k][1] 的值。
     *  最终答案是 T[n - 1][k][0], 因为结束时持有 0 份股票的收益一定大于 持有 1 份股票的收益。
     *
     * 二、应用于特殊情况
     * 上述 6 个股票问题是根据 k 的值进行分类的，其中 k 是允许的最大交易次数。最后两个问题有附加限制，包括【冷冻期】 和 【手续费】。
     * 通解可以应用于每个股票问题。
     * 情况一： k = 1
     *      情况一，对应的题目是【121. 买卖股票的最佳时机】
     *      对于情况一，每天有两个未知变量：T[i][1][0] 和 T[i][1][1],状态转移方程如下：
     *          T[i][1][0] = max(T[i - 1][1][0], T[i - 1][1][1] + prices[i])
     *          T[i][1][1] = max(T[i - 1][1][1], T[i - 1][0][0] - prices[i]) = max(T[i - 1][1][1],-prices[i])
     *      第二个转移方程利用了 T[i][0][0] = 0。
     ...
     *             // dp[i - 1][0] - prices[i] ,因为 k = 1，故当第 i - 1天为 0 时，收益为 0.dp[i - 1][0] = 0
     *         }
     ...
     *         return profit0;
     *     }
     *     现在对上述解法进行分析。对于循环中的部分，profit1实际上只是表示到第 i 天的股票价格的相反数中的最大值，或者等价地表示到第 i 天的股票价格的最小值。
     *     对于profit0，只需要决定卖出和休息中的哪项操作可以得到更高的收益。
     *     如果进行卖出操作，则买入股票的价格为profit1，即第 i 天之前（不含第 i 天）的最低股票价格。
     *     这正是现实中为了获得最大收益会做的事情。但是这种做法不是唯一适用于这种情况的解决方案。读者可能在 这里 找到别的好的解决方案。
     *
     *  情况二： k 为正无穷
     *  情况二对应的题目为 122. 买卖股票的最佳时机 II
     *  如果 k 为正无穷，则 k 和 k - 1可以看成是相同的，因此有 T[i - 1][k - 1][0] = T[i - 1][k][0] 和
     *  T[i - 1][k - 1][1] = T[i - 1][k][1]。每天仍有两个未知变量：T[i][k][0] 和 T[i][k][1],其中 k 为正无穷，状态转移方程如下：
     *      T[i][k][0] = max(T[i - 1][k][0],T[i - 1][k][1] + prices[i])
     *      T[i][k][1] = max(T[i - 1][k][1],T[i - 1][k - 1][0] - prices[i]) = max(T[i - 1][k][1],T[i - 1][k][0] - prices[i])
     *  第二个状态转移方程利用了 T[i - 1][k - 1][0] = T[i - 1][k][0]
     *  根据上述状态转移方程，可以写出时间复杂度 O(n) 和 空间复杂度 O(n) 的解法。
     *     public int maxProfit(int[] prices){
     *         if (prices == null || prices.length == 0){
     *             return 0;
     *         }
     ...
     *         return profit0;
     *     }
     *    这个解法提供了获得最大收益的贪心策略：可能的情况下，在每个局部最小值买入股票，然后在之后遇到的第一个局部最大值卖出股票。
     ...
     *     }
     *  情况四：k为任意值
     *     情况四对应的题目为 188. 买卖股票的最佳时机 IV
     *     情况四是最通用的情况，对于每一天需要使用不同的 k 值更新所有的最大收益，对应持有 0 份股票 或 1 份股票
     *     如果 k 超过 一个临界值，最大收益就不再取决于允许的最大交易次数，而是取决于股票价格数组的长度，因此可以进行优化。那么这个临界值是什么呢？
     ...
     *      由于具有相同的 k 值，因此情况五 和 情况二 非常相似，不同之处在于 情况五有 冷却时间 的限制，因此需要对状态转移方程进行一些修改。
     *      情况二的状态转移方程如下：
     *          T[i][k][0] = max(T[i - 1][k][0],T[i - 1][k][1] + prices[i])
     *          T[i][k][1] = max(T[i - 1][k][1],T[i - 1][k][0] - prices[i])
     *      但是在有 冷却时间 的情况下，如果在第 i - 1天卖出了股票，就不能在第 i 天买入股票。
     *      因此，如果要在第 i 天买入股票，第二个状态转移方程中就不能使用 T[i - 1][k][0]，而应该使用 T[i - 2][k][0].
     ...
     *    由于具有相同的 k 值，因此情况六和情况二非常相似，不同之处在于 情况六有手续费 ，因此需要对状态转移方程进行一些修改。
     *    情况二的状态转移方程如下:
     *      T[i][k][0] = max(T[i - 1][k][0] , T[i - 1][k][1] + prices[i])
     *      T[i][k][0] = max(T[i - 1][k][1] , T[i - 1][k][0] - prices[i])
