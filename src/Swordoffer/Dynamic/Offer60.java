...
public class Offer60 {
    ...
    public static void main(String[] args) {
        Offer60 offer = new Offer60();
        System.out.println(Arrays.toString(offer.dicesProbability(2)));
    }

    /**
     * 题解链接：https://leetcode.cn/problems/nge-tou-zi-de-dian-shu-lcof/solution/jian-zhi-offer-60-n-ge-tou-zi-de-dian-sh-z36d/
     * 方法一：动态规划
     * 复杂度分析：
     *   时间复杂度O(n^2)：状态转移循环 n-1 0轮；每轮中，当 i=2,3,..,n时，对应循环数量分别为 6×6,11×6,...,[5(n−1)+1]×6；
     *   因此总体复杂度为 O((n-1) * ((6+[5(n-1)+1])*6/2)),即等价于 O(n^2)。
     *   空间复杂度O(n):状态转移过程中，辅助数组tmp最大长度为 6(n−1)−[(n−1)−1]=5n−4，
     *   因此使用 O(5n - 4) = O(n) 大小的额外空间。
     * @param n
     * @return
     */
